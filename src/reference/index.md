...
| `STAFF_MODIFY_LOGLEVEL`       | `1048576` | Grants access to modify the log level of the server.                                                                                                                                                                                 |

## Log Levels

The Cumulonimbus API has a number of log levels that can be set by users with staff permissions. These log levels control the amount of information that is logged by the server. The available log levels are:

- `DEBUG` - Logs all information, including debug messages. This log level is intended for use in development environments.
- `INFO` - Logs general information about the server and its operation. This is the default log level.
- `WARN` - Logs warnings about potential issues or errors that may occur.
- `ERROR` - Logs errors that occur during the operation of the server.
- `NONE` - Disables all logging. This really should never be used, but it is available for those who want to disable logging entirely.
